# Instead of running the full pipeline on all possible WoS input records,
# this focuses on only the ones that were not fully merged with 
# Semantic Scholar (S2) data
...
from vespid.pipeline import PipelineFlag, Stage, Pipeline, ParallelPipeline
...
def setup_stages(
    ...
    *Serial-Executed*
    ...
    def _load_pickled_s3_object(bucket, key, ids_to_retain=None):
        output = pickle.loads(get_s3_file(bucket, key))
        
        # Drop the records from `df` that don't match our missing ID list
        if ids_to_retain is not None:
            output = output[output['id'].isin(ids_to_retain)].copy()
            
        # Make sure we don't end up with an empty DataFrame! If so, exit the process
        if output.empty:
            return Pipeline.cancel(return_value=None)
        
        # Track the input filename for forensic purposes
        input_filename = os.path.split(key)[-1]
        output['source'] = input_filename
        logger.info(f"Ingesting file {input_filename}...")
    ...
    def _pass_through_data(data):
        ...
        if data.empty:
            raise RuntimeError("No data passed to the graph Pipeline!")
...
def main(
    ...
    # Query the target graph to get the records missing S2 IDs and thus 
    # needing to still be integrated
    target_graph = Neo4jConnectionHandler(
        db_ip=db_ip,
        db_username=db_username,
    ...
    """
    papers_missing_s2_ids = target_graph.cypher_query_to_dataframe(
        query,
        verbose=True
    )['id_wos']
    ...
    for batch_index in tqdm(
        ...
        parallel_stages, serial_stages = setup_stages(
            None,
            wos_ids_of_interest=papers_missing_s2_ids,
            db_ip=db_ip,
            db_password=db_password,
            db_username=db_username,
            s3_bucket=s3_bucket,
